...
function resolveOtherSellersForVariant(multiOffer, variant, productMeta) {
  ...
  if (ean && byEan) {
    ...
    // Known EAN with no bucket / empty → no other sellers for this child
    if (Object.prototype.hasOwnProperty.call(byEan, ean)) return [];
  }
  // Fallback for older API payloads without by_ean
...
export default function ProductTemplate() {
  ...
  // metadata.seller_id, which is only populated for some listings — falling back to it
  // alone silently left this stuck on the platform default store name for products where
  // metadata.seller_id was never written.
  useEffect(() => {
  ...
  const handleBuyNow = async () => {
    ...
    try {
      await addToCart(variantId, quantity, product.seller_id || null);
    } catch (_) {}
